...
function Booking() {
  ...
  useEffect(() => {
    if (mechanic) {
      ...
      const slots = mechanic.availableTimeSlots
        ? mechanic.availableTimeSlots.split(",")
        : [];
      setAvailableTimeSlots(slots);

      // Split the first element of the skills array into individual skills
      const mechanicSkills =
        mechanic.skills.length > 0 ? mechanic.skills[0].split(",") : [];
  ...
  return (
    <div
      className="container mx-auto mt-10 mb-5 border border-black border-2 border-opacity-25"
      style={{
        width: "400px",
        boxShadow: "0 0 20px green, 0 0 5px black inset",
      }}
    >
      {/* <Profile mechanic={mechanic} /> */}
      <div className="title flex flex-col items-center">
        <h1 className="text-3xl font-bold text-black-500">Book a Mechanic</h1>
      ...
      <form className="py-1" onSubmit={handleSubmit}>
        <div className="text-box flex flex-col items-center gap-6 justify-items-center">
          ...
          >
            <button
              className="btn text-black px-2 py-2 rounded border-black"
              type="submit"
              style={{
                marginBottom: "15px",
                width: "100px",
