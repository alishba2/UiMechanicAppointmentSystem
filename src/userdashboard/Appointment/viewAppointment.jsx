...
export default function ViewAppointment({ user }) {
    ...
    useEffect(() => {
        if (user && user._id) {
            axios.get(`http://localhost:3001/appointments/user/${user._id}`)
                .then(async (response) => {
                    ...
                    for (const appointment of appointmentsData) {
                        if (appointment.mechanicId && !usernames[appointment.mechanicId]) {
                            const userResponse = await axios.get(`http://localhost:3001/user/${appointment.mechanicId}`);
                            usernames[appointment.mechanicId] = userResponse.data.user.username;

                            // Split availableTimeSlots string into an array
                            const slotsArray = userResponse.data.user.availableTimeSlots.split(',');
    ...
    const handleConfirmReschedule = () => {
        if (selectedAppointment && newTimeSlot) {
            axios.put('http://localhost:3001/appointments/reschedule', {
    ...
    const handleConfirmCancel = () => {
        if (selectedAppointment) {
            axios.put('http://localhost:3001/appointments/status', {
                appointmentId: selectedAppointment._id,
                status: 'Cancelled'
            })
                .then(response => {
                ...
                <tbody>
                    {appointments.map((appointment, index) => (
                        <tr key={appointment._id}>
                            ...
                            <td>{mechanicUsernames[appointment.mechanicId] || 'Loading...'}</td>
                        ...
                        <Form>
                            <Form.Group controlId="formNewTimeSlot">
                                ...
                                >
                                    ...
                                    {(availableSlots[selectedAppointment?.mechanicId] || []).map((slot, index) => (
